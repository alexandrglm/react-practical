...
export default function CalculationScreen({route}) {
  const {operation, questionCount = 5 } = route.params;
  ...
  const remaining = () => answers.filter(a => a).length + '/' + questionCount;
        ...
        {result !== null
          ...
          <Stack spacing={2} style={{ margin: 16 }}>
            ...
            { 
              ...
              :
              <Results answers={answers} questionCount={questionCount}/>
            }
  ...
  const Results = ({answers, questionCount}) => {
    return (
