...
function App() {
  ...
  useEffect( () =>{
    axios.get('/player').then(result => {setTeam(result.data)});
  }, [])
  ...
  const savePlayer = player => {
    if (player.id)
      axios.put(`/player/${player.id}`, player).then(result => {
        saveInState(result.data);
      });
    else
      axios.post('/player', player).then(result => {
        saveInState(result.data);
      });
  };
  ...
  const handleDelete = player => {
    axios.delete(`/player/${player.id}`, player).then(result => {
      const player = result.data;
      console.log("Handling delete: ", result.data)
      const filteredTeam = team.filter(member => member.position !== player.position)
